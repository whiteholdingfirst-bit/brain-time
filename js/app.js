/* =========================================================
   BRAIN TIME — interfaccia e navigazione
   ========================================================= */
(function (BT) {
  'use strict';

  var player = null;          // giocatore attivo
  var mode = 'solo';          // solo | duel
  var tipo = 'gioca';         // allena (senza punti) | gioca (con punti e penalita')
  var duelSel = [null, null]; // sfidanti scelti
  ...
  BT.show = function (id, noHistory) {
    ...
    // durante il gioco e il passaggio del dispositivo niente tasto indietro
    var back = document.getElementById('btn-back');
    back.style.visibility = (id === 'screen-game' || id === 'screen-pass') ? 'hidden' : 'visible';
  ...
  function goBack() {
    ...
    if (prev === 'screen-game' || prev === 'screen-pass' || prev === 'screen-result')
  ...
  BT.refreshTopbar = function () {
    ...
    el.innerHTML = '<span>' + BT.avatar(player.avatar) + '</span>' +
  ...
  BT.renderHome = function () {
    ...
    list.forEach(function (p) {
      ...
      row.innerHTML =
        ...
        '<div class="pf-info">' +
          '<div class="pf-name">' + BT.esc(p.name) + '</div>' +
          '<div class="pf-meta">Livello ' + lvl + ' &middot; ' + BT.levelTitle(lvl) +
          ' &middot; ' + p.coins + ' 🏆</div>' +
        '</div>' +
        '<div class="pf-lvl">' + BT.classeBreve(p) + '</div>' +
        '<button class="pf-del" title="Elimina">&times;</button>';

      row.onclick = function () { selectPlayer(p); };
      row.querySelector('.pf-del').onclick = function (ev) {
        ev.stopPropagation();
        if (confirm('Vuoi davvero eliminare il giocatore ' + p.name + '? I punti andranno persi.')) {
          BT.store.remove(p.id);
          BT.renderHome();
        }
      };
      box.appendChild(row);
  ...
  /* Le preferenze del dispositivo (difficolta', suoni) non vanno pubblicate:
     online un salvataggio ripubblica la pagina e la fa ricaricare sotto le
     mani di chi sta giocando. Qui basta scriverle sul dispositivo. */
  ...
  function renderNewProfile(esistente) {
    ...
    /* --- personaggi, divisi per famiglia --- */
    ...
    document.getElementById('np-hint').innerHTML =
      'L&rsquo;et&agrave; serve solo a scegliere la difficolt&agrave;. In classifica i giocatori restano ' +
      'confrontabili: le domande sono diverse, ma i punti valgono uguale.' +
      (esistente ? '<br>Punti, coppe e sblocchi restano come sono: cambiano solo questi dati.' : '');
  }
  ...
  BT.renderMenu = function () {
    ...
    document.getElementById('menu-player-card').innerHTML =
      '<div class="pc-top">' +
        ...
        '<div>' +
          '<div class="pc-name">' + BT.esc(player.name) + '</div>' +
  ...
  function renderCategories() {
      ...
      carta('cat-extra', '🌍', 'Impara una lingua', 'Parole e frasi, scegli tu la lingua', function () {
        BT.lingue.scegli(function (lin) {
          chiediDifficolta('Poi si comincia con ' + BT.lingua(lin).nome.toLowerCase() + '.',
            function (df) {
              BT.lingue.start({ player: player, tipo: tipo, lingua: lin, diff: df });
            });
        });
      });
    ...
    document.getElementById('cat-title').innerHTML = mode === 'duel'
      ? 'Materie del duello'
      : (tipo === 'allena' ? 'Materie dell\'allenamento' : 'Scegli la materia');
  ...
  function launch(cats) {
    var players = mode === 'duel' ? [duelSel[0], duelSel[1]] : [player];
    BT.sfx.play('click');
    BT.game.start({
      mode: mode,
      tipo: mode === 'duel' ? 'gioca' : tipo,
      players: players,
      cats: cats,
      diff: BT.store.settings().diff
    });
  }

  BT.startAllena = function () {
    mode = 'solo'; tipo = 'allena';
    BT.show('screen-category');
    renderCategories();
  };

  BT.startGioca = function () {
    mode = 'solo'; tipo = 'gioca';
    BT.show('screen-category');
    renderCategories();
  };


  /* compatibilita' con i richiami piu' vecchi */
  ...
  function renderRank() {
    ...
    if (!list.length) {
      box.innerHTML = '<p class="empty-note" style="color:#fff">Ancora nessun giocatore.</p>';
      return;
    }
    ...
    list.forEach(function (p, i) {
      ...
      row.innerHTML =
        ...
        '<div class="rank-info">' +
          '<div class="rank-nm">' + BT.esc(p.name) + ' <small style="opacity:.55">' +
            BT.classeBreve(p) + '</small>' +
          ...
          '<div class="rank-meta">Liv. ' + lvl + ' &middot; precisione ' + acc + '% &middot; ' +
            p.answered + ' domande &middot; duelli vinti ' + p.duelsWon + '</div>' +
        '</div>' +
    ...
    document.getElementById('rank-hint').innerHTML = rankMode === 'brain'
      ? 'I <b>Punti Cervello</b> premiano chi gioca tanto e risponde bene.'
  ...
  function renderStats() {
    var acc = player.answered ? Math.round(player.correct / player.answered * 100) : 0;
    var html = '<div class="card">' +
      '<h2 class="card-title">' + BT.avatar(player.avatar) + ' ' + BT.esc(player.name) + '</h2>' +
      '<div class="res-stats">' +
        '<div class="res-stat"><b>' + player.answered + '</b><span>domande giocate</span></div>' +
        '<div class="res-stat"><b>' + acc + '%</b><span>precisione</span></div>' +
        '<div class="res-stat"><b>' + player.bestRun + '</b><span>record in una partita</span></div>' +
      '</div>' +
      '<div class="res-stats" style="margin-bottom:0">' +
        ...
        '<div class="res-stat"><b>' + player.coins + '</b><span>coppe</span></div>' +
      '</div></div>';
    ...
    if (!giocate.length) {
      ...
      righe.forEach(function (r) {
        ...
        var col = r.perc < 0 ? 'var(--azzurro-100)'
                : r.perc >= 75 ? 'var(--verde)'
                : r.perc >= 50 ? 'var(--oro)' : 'var(--rosso)';
        html += '<div class="bar-row">' +
          '<div class="bar-head"><span>' + r.c.ico + ' ' + r.c.name + '</span>' +
          '<span>' + (r.perc < 0 ? 'mai giocata' : r.ok + '/' + r.a + ' &middot; ' + r.perc + '%') + '</span></div>' +
          '<div class="bar-track"><div class="bar-fill" style="width:' + perc + '%;background:' + col + '"></div></div>' +
          '</div>';
      });
  ...
  function esportaBackup() {
    try {
      ...
      a.href = url;
      a.download = 'brain-time-backup-' + stamp + '.json';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(function () { URL.revokeObjectURL(url); }, 1000);
      document.getElementById('set-msg').innerHTML =
        'Backup salvato nella cartella dei download. Conservalo: contiene tutti i profili e i punteggi.';
    } catch (e) {
      document.getElementById('set-msg').textContent = 'Non sono riuscito a salvare il backup: ' + e.message;
    }
  }
  ...
  function importaBackup(file) {
    ...
    r.onerror = function () { document.getElementById('set-msg').textContent = 'Non riesco a leggere il file.'; };
    r.readAsText(file);
  }
  ...
  function apriLabirinti() {
    ...
    for (var i = 1; i <= massimo; i++) {
      (function (liv) {
        ...
        b.disabled = !aperto;
        b.onclick = function () { BT.laby.gioca(player, liv, { allena: tipo === 'allena' }); };
        lista.appendChild(b);
      })(i);
    }
  ...
  /* chiamata da laby.js quando il labirinto finisce */
  BT.laby.mostraEsito = function (e) {
    ...
    if (b2) b2.onclick = function () { BT.laby.gioca(player, e.livello + 1, { allena: tipo === 'allena' }); };
    var b3 = box.querySelector('#lb-riprova');
    if (b3) b3.onclick = function () { BT.laby.gioca(player, e.livello, { allena: tipo === 'allena' }); };
    var b4 = box.querySelector('#lb-cassa');
    if (b4) b4.onclick = function () { BT.mostraCassa(player); };
  };

  /* ================= la sfida a distanza =================
     Le sfide vivono nella pagina online: e' l'unico posto dove due
     dispositivi diversi si vedono. Da qui ci si arriva, ma non si puo'
     mostrarla dentro questa finestra: claude.ai non si lascia incorniciare
     da una pagina aperta come file (e' una regola dei browser, non una
     scelta nostra). Quindi si apre, spiegando prima che cosa succede. */
  /* L'indirizzo della pagina online NON sta nel codice: questo repository e'
     pubblico, e un indirizzo resta un indirizzo anche quando la pagina dietro
     chiede di autenticarsi. Si prende, in ordine:
       1. da js/config-locale.js (fuori dal repository), se qualcuno l'ha messo;
       2. da quello che il giocatore ha incollato una volta su questo dispositivo.
     Se non c'e' ne' l'uno ne' l'altro, la tessera lo chiede una volta sola. */
  var CHIAVE_URL = 'brain-time-url-online';

  function urlOnline() {
    if (BT.URL_ONLINE) return BT.URL_ONLINE;
    try { return localStorage.getItem(CHIAVE_URL) || ''; } catch (e) { return ''; }
  }
  function salvaUrlOnline(u) {
    try { localStorage.setItem(CHIAVE_URL, u); } catch (e) {}
  }
  ...
  function apriOnline() {
    document.getElementById('pausa-box').innerHTML =
      '<div class="res-card">' +
        '<div class="res-ico">📡</div>' +
        '<div class="res-title">Sfida a distanza</div>' +
        '<div class="res-sub">Le sfide si giocano nella <b>versione online</b>: &egrave; l&rsquo;unico ' +
          'posto dove il tuo gioco e quello di chi &egrave; lontano si vedono fra loro. ' +
          'L&igrave; trovi anche la classifica di tutti.</div>' +
        '<div class="res-reward">&Egrave; lo stesso gioco con gli stessi giocatori. ' +
          'I punti fatti qui sul computer restano qui: le due versioni tengono due elenchi ' +
          'separati, perch&eacute; questa funziona anche senza internet.</div>' +
        '<div class="row-btns">' +
          '<button class="btn btn-ghost" id="on-no">Torna indietro</button>' +
          '<button class="btn btn-primary" id="on-vai">Apri il gioco online</button>' +
        '</div>' +
      '</div>';
    BT.show('screen-pausa');
    document.getElementById('on-no').onclick = function () { BT.show('screen-menu'); BT.renderMenu(); };
    document.getElementById('on-vai').onclick = function () {
      BT.sfx.play('click');
      var u = urlOnline();
      if (!u) {
        u = (prompt('Incolla qui l\'indirizzo della vostra pagina online.\n' +
                    'Lo chiedo una volta sola: resta su questo dispositivo.') || '').trim();
        if (!u) return;
        if (u.indexOf('http') !== 0) { BT.toast('Non sembra un indirizzo valido.'); return; }
        salvaUrlOnline(u);
      }
  ...
  /* Cassa messa da parte: prima si tornava al menu da soli, e non si faceva
     in tempo a leggere quanto varra' la prossima. Adesso si esce a comando,
     come per i premi. */
  ...
  /* L'elenco dei premi non se ne va da solo e non ha nessun timer: resta
     finche' il giocatore non preme "Ho letto". Serve davvero, perche' nella
     versione online ogni salvataggio ripubblica la pagina e la pagina si
     ricarica: i premi sono scritti nel profilo, quindi questa schermata
     ricompare da sola dopo il ricaricamento e sparisce solo a comando. */
  ...
  function init() {
      ...
      BT.foto.accendi(
        ...
        function (motivo) {
          ...
          if (motivo === 'permesso-negato') {
            ...
            // su telefono e tablet questo apre direttamente l'app fotocamera
    ...
    document.querySelectorAll('.tile').forEach(function (t) {
      t.onclick = function () {
        var go = t.dataset.go;
        if (go === 'gioca') { if (controllaLimite()) { mode = 'solo'; tipo = 'gioca'; BT.show('screen-category'); renderCategories(); } }
        else if (go === 'allena') { if (controllaLimite()) { mode = 'solo'; tipo = 'allena'; BT.show('screen-category'); renderCategories(); } }
        else if (go === 'online') { apriOnline(); }
        else if (go === 'scoperte') { renderScoperte(); }
        else if (go === 'duel') {
          if (!controllaLimite()) return;
          if (BT.store.all().length < 2) {
            BT.toast('Per il duello servono almeno due giocatori. Creane un altro!');
            return;
          }
          mode = 'duel';
          BT.show('screen-duel-setup');
          BT.renderDuelSetup(true);
        }
    ...
    if (bm) bm.onclick = function () {
      ...
      BT.store.save ? BT.store.save() : BT.store.salvaPref();
    ...
    document.getElementById('set-file').onchange = function () {
      if (this.files && this.files[0]) importaBackup(this.files[0]);
      this.value = '';
    };
    ...
    document.addEventListener('keydown', function (e) {
      var gameOn = document.getElementById('screen-game').classList.contains('active');
      if (!gameOn) return;
      if (e.key >= '1' && e.key <= '4') {
        ...
        var nx = document.getElementById('fb-next');
        if (nx) nx.click();
      }
    });
    ...
    if (!BT.store.canSave()) {
      setTimeout(function () {
        BT.toast('Attenzione: questo browser non salva i progressi. Prova ad aprire il file con Chrome o Edge.', 6000);
      }, 800);
    }
  }
